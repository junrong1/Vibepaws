...
| 9 | **Resize / scale** | WindowPet, vscode-pets (nano/med/large), Desktop Mate | ✅ **shipped 9/5** — three sizes in Settings and the tray, applied as a zoom factor on the whole window so pet, bubbles and flyout scale together |
| 10 | **Multi-monitor handling** | partial everywhere; the #1 recurring complaint in the genre | ✅ **shipped 9/5** — position remembered per display under a key that survives a replug, follow-the-cursor with a 1.2s dwell or pin to one screen, and an active reconcile on plug/unplug so the pet is never stranded on a display that is gone |
| 11 | **Auto-start on login** | WindowPet, Tamamon, tama96 | ✅ **shipped 9/5** — `npm start` is the whole launch; Settings → Startup (or the tray) registers a login item that brings up pet and Core together |
| 12 | **Auto-update** | WindowPet, Tamamon (GitHub Releases) | ❌ |
| 13 | **Settings window** | all | ✅ **shipped 8/22** — pet name, token budget (global + per session), context thresholds, daily EXP cap, per-session goals, language. Autostart, pet size and multi-monitor landed 9/5; the sound row arrives with 0.2 |
...
| 20c | ~~**Subagent-aware states**~~ ✅ — 1 subagent vs. 2+ render differently | clawd (groove / juggling) | ✅ **shipped 9/13** — `delegating` (1) / `juggling` (2+) as tiers of `working`, counted desk-wide, no new art: borrowed portrait + motion + one orbiting square per subagent, exact count in the flyout |
...
- **Anti-virus and Gatekeeper.** Desktop pets get flagged constantly; clawd [#872](https://github.com/rullerzhou-afk/clawd-on-desk/issues/872) is about macOS requiring a Privacy & Security click on *every* update, and openpets went as far as **SignPath-signed builds**. Vibepaws already documents the unsigned-build problem in its README — this confirms signing is not optional if it ships beyond friends. **Done (0.13, 9/4)** — and the dmg turned out to be the half that actually produces #872's symptom: electron-builder staples the `.app` only, so the warning survives right up until the disk image is notarized on its own.
...
| ~~Subagent completion mistaken for main-task completion~~ ✅ pinned by a test (0.11, 9/13) | clawd #214 |
| ~~Subagent tier fails to escalate from 1 to 2+~~ ✅ pinned by a test (0.11, 9/13) | clawd #862 |
...
| 0.3 | ~~**Pet scale + multi-monitor**~~ ✅ | **Shipped 9/5.** Three sizes (0.8 / 1 / 1.3) applied as a zoom factor on the whole window, so pet, bubbles and flyout scale together and no number in the CSS has to remember to multiply; position remembered per display; follow the active display or pin to one. Both are in Settings and in the tray. Four things only surfaced by building it. First, **`display.id` is the wrong key** — macOS renumbers `CGDirectDisplayID` across unplug, port change and reboot, and recognising a display *across a replug* is the entire value of remembering position per display; the key is model + resolution + scale instead, with two identical monitors (a very common dual setup) disambiguated by physical placement — so swapping them in System Settings swaps the memory too, which is right: what a user remembers is "the bottom-right of the left screen", not a serial number. Second, **positions have to be stored as work-area fractions, not coordinates** — an absolute point is only valid while the resolution never changes, and one scaling change, one projector or one macOS default-resolution shift puts it off-screen. That is the actual mechanism behind *"the pet vanished when I switched monitors"*, and it is why an unplug needs an active reconcile: the window keeps its coordinates on a desktop that no longer has pixels, Electron will not pull it back, and the tray still reads perfectly normal. Third, **follow needs a dwell, not an edge trigger** — without the 1.2s settle every cross-screen mouse sweep drags the pet along, and sweeping between screens is something a multi-monitor user does every minute; a pet that chases the cursor is worse than one that sits still. Fourth, **a pinned display that gets unplugged stays pinned** — the pet falls back to primary so it is visible, but the pin itself is not rewritten, or "stay on one display" quietly changes meaning after every lid-close; Settings says where the pet went in the meantime. All of it is pure functions that never import Electron, because every one of these only goes wrong on someone else's desk | ~~3d~~ **done** | ★ |
| 0.4 | ~~**Autostart + single-command launch**~~ ✅ | **Shipped 9/5.** `npm start` is the whole launch: the tray app finds a Node ≥ 22.6, spawns Core, watches it (five restarts, exponential backoff, then it *stops* rather than loop), and reaps it on quit — plus a login item in Settings → Startup and in the tray. Three things only surfaced by building it. First, **an already-running Core has to be adopted, not owned**: `npm run core:watch` in the next terminal is the dev workflow, and killing it on quit would kill the thing the developer is watching — so an adopted Core is used, never reaped, and *health-polled*, because a process you did not spawn sends you no exit event; without the poll, one Ctrl-C in that terminal left the tray reading "running" over a dead daemon. Second, **`PATH` is the wrong place to look for Node** — the launch that matters here is the login item, and launchd hands the app `/usr/bin:/bin:/usr/sbin:/sbin`, where Homebrew, nvm, fnm, Volta and `n` all live outside it; "starts at login" would otherwise have meant "shows Core offline every morning". Third, the login item **cannot be exercised from a dev run at all** — it would register `node_modules/electron` — so the toggle disables itself and says why, as it also does when the app is still running from the mounted dmg, where the login item dies with the eject | ~~2d~~ **done** | ★ |
...
| **0.11** | ~~**Subagent-aware states**~~ ✅ | **Shipped 9/13.** `sessions.subagent_count` is maintained by `SubagentStart`/`SubagentStop` and splits `working` into three tiers — 0 self, 1 `delegating`, 2+ `juggling` — aggregated across the whole desk, not per session, so two sessions with one subagent each is two subagents running and the pet juggles (that per-session reading is exactly what #862 got wrong). Neither tier needed new art: they borrow the `working` portrait and carry the difference in motion — `delegating` is deliberately **slower** than `working`, because the job went out and the pet is supervising — plus one orbiting square per subagent, capped at five, with the exact count in the flyout (`Juggling ×4`). Three things only surfaced by building it. First, **a count drifts and the render has to survive that**: a missed `SubagentStop` (hook timeout, `kill -9`) never comes back, so there are four independent drains — the decrement clamps at 0, session lifecycle events zero it, and the tiers are a *refinement of* `working` rather than a peer of it, which means a session that hasn't worked in 15 minutes shows `idle` no matter what the counter says. That last one is why the feature needs no timeout constant of its own. Second, **#214 has a delayed form nobody documents**: not honouring a `Stop` while subagents run is the easy half, but if you keep that marker and re-honour it once the count drains, the pet announces "done, waiting for you" at the exact instant the last subagent returns — the same bug, one event later. A ready marker that ever coexisted with running subagents is discarded, and the session falls back to `working`; a genuine turn end re-arms it, and missing one `ready` is far cheaper than one false "finished". Third, the orbit is **the only effect that leaves the sprite's bounding box**, so it has to be sized against the canvas rather than the sprite: a 160px pet centred on a 208px canvas leaves 24px a side, and the obvious "3 cells outside the box" radius put the squares through the window edge — where they don't clip, they vanish, which reads as the effect randomly dropping frames. Caught by a test that samples every pet at every phase of both motions. | ~~2d~~ **done** | ★★ |
...
| **0.13** | ~~**Code signing + notarization**~~ ✅ | **Shipped 9/4.** Hardened runtime, minimal entitlements (every key commented with what breaks without it), notarization of the `.app` *and* the `.dmg`, and a tagged CI release. Two things only surfaced by building it. First, **electron-builder notarizes the `.app` but not the `.dmg`** — the app inside is stapled, so dragging it out works, but double-clicking the dmg is evaluated against the dmg's own (absent) ticket. That gap *is* clawd #872's "Privacy & Security on every update", so `build/notarize-dmg.cjs` staples the dmg separately. Second, and the reason the deliverable is a verifier rather than a config diff: **signing failures are invisible locally** — your own build never gets a quarantine flag, so Gatekeeper never evaluates it and a completely broken signature launches fine forever. `npm run verify:release` asks macOS directly, and walking every Mach-O in the bundle immediately found `better-sqlite3` shipping **twice** (asar + extraResources): 26 MB of dead weight nobody required, doubling the signing surface from 19 binaries to 16 after the fix, and 12 MB off the download | ~~2d~~ **done** | ★ |

**Tier 0 total: ~16 days left** (28.5, minus 0.1 and 0.5 shipped 8/22, 0.10 and 0.12 shipped 8/25, 0.13 shipped 9/4, 0.4 shipped 9/5, and 0.11 shipped 9/13). Nothing here is differentiating; all of it is disqualifying if absent — and 0.9 is the item a user will notice within one session.
...
1. **Ship Tier 0** (~16d left — 0.1 and 0.5 shipped 8/22, 0.10 and 0.12 shipped 8/25, 0.13 shipped 9/4, 0.4 shipped 9/5, 0.11 shipped 9/13). Not optional. A silent, un-resizable, two-terminal-command pet with no approve button loses on polish before anyone evaluates the growth loop. **Start with 0.9 (actionable permission bubbles)** — a notification you have to leave the app to act on is a half-built feature, and it's the clearest thing clawd has that Vibepaws doesn't.
