...
| 20d | **Zombie-session reclamation** — detect crashed agent processes, clean up orphan sessions | clawd (process liveness detection) | ❌ open as **G10** in the internal gap analysis; `is_active` never resets |
...
**Note on G10 and G19**: both are listed as open blockers in `references/architecture_gap_analysis.md`, and clawd has shipped solutions to both. They are solvable, and someone has proven it.
...
| **0.10** | **Zombie-session reclamation** | Process liveness detection + orphan cleanup. Internal gap **G10**; clawd has shipped a solution, so it's tractable | 2d | ★ |
...
| **0.12** | **"It can't eat your tokens" trust copy** | A user genuinely believed the pet was burning their tokens because Claude *hallucinated* it (clawd #102). Every hook-based pet hits this. One line in the README, one in the app, plus a visible hook byte/latency counter. Half a day to prevent a recurring trust problem | 0.5d | ★★ |
...
**Tier 0 total: ~22 days left** (26, minus 0.1 and 0.5, both shipped 8/22). Nothing here is differentiating; all of it is disqualifying if absent — and 0.9 is the item a user will notice within one session.
