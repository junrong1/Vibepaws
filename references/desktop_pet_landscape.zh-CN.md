...
| **僵尸会话回收** —— 检测崩掉的 agent 进程、清理孤儿会话 | ❌ 内部 gap 分析里的 **G10**，`is_active` 永不复位 |
| **终端聚焦 / 真正的跳回会话** | ⚠️ **G19**；Vibepaws 目前只是复制一条命令到剪贴板 |
| **手机会话镜像** | ❌ |
| **迷你 / 贴边模式** | ❌ |
| **第三方主题包导入** | ⚠️ 只支持自己的管线 |

> **关于 G10 与 G19**：两者都是内部 gap 分析里挂着的 open blocker，而 clawd 都已经交付了解决方案。所以它们是**可解的，而且有人证明了**。
...
| **0.10** | **僵尸会话回收** | 进程存活检测 + 孤儿清理。内部 gap **G10**；clawd 已交付解法，所以可行 | 2 | ★ |
