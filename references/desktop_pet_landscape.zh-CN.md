...
| **缩放 / 尺寸** | WindowPet、vscode-pets、Desktop Mate | ✅ **9/5 已交付** —— 设置与托盘里的三档尺寸，作用在整扇窗口上，宠物、气泡、浮层一起缩放 |
| **多显示器处理** | 到处都只是部分支持 —— 全品类第一大抱怨 | ✅ **9/5 已交付** —— 位置按显示器分别记住（键跨拔插稳定），跟随光标（1.2 秒停留）或固定在一块屏上，拔插后主动重新落位 —— 宠物不会被丢在一块已经不在的屏上 |
| **开机自启** | WindowPet、Tamamon、tama96 | ✅ **9/5 已交付** —— `npm start` 一条命令启动全部；设置 → 启动（或托盘）注册登录项，宠物和 Core 一起拉起 |
| **自动更新** | WindowPet、Tamamon | ❌ |
| **设置窗口** | 全部 | ✅ **8/22 已交付** —— 宠物名、token 预算（全局 + 按 session）、上下文阈值、每日 EXP 上限、按 session 的目标、界面语言。自启、宠物大小、多显示器均于 9/5 落地；音效那一行等 0.2 |
...
| ~~**subagent 感知状态**~~ ✅ —— 1 个和 2+ 个渲染不同 | ✅ **9/13 已交付** —— `delegating`（1）/ `juggling`（2+）是 `working` 的两档细分，按整张桌面计数，没有新立绘：借立绘 + 动作 + 一个 subagent 一颗绕圈方块，确切数字在浮层里 |
...
| ~~把 subagent 完成误判为主任务完成~~ ✅ 已由测试钉住（0.11，9/13） | clawd #214；#862 里 1→2+ 分档未升级 |
...
| # | 功能 | 说明 | 天 | 差异化 |
| --- | --- | --- | --- | --- |
| 0.1 | ~~**设置窗口**~~ ✅ | **8/22 已交付**，做成一扇独立窗口 —— 宠物窗口只有 300px 宽还要放过点击，那里放不了表单。宠物名 · token 预算（全局 + 按 session）· 上下文阈值（可关）· 每日 EXP 上限 · 按 session 的目标 · 语言。两个依赖手改 SQLite 的 Activation 指标现在都能从界面走到，`topic` 倍率与漂移基准也终于有了录入口（**G17**）。自启、宠物大小、多显示器已于 9/5 各占一行；音效那一行等 0.2 交付 —— 那些功能不存在之前，这里没有东西可配 | ~~3~~ **已完成** | ★ |
...
| 0.3 | ~~**缩放 + 多显示器**~~ ✅ | **9/5 已交付。** 三档尺寸（0.8 / 1 / 1.3）作用在整扇窗口的 zoom factor 上，宠物、气泡、浮层一起缩放，CSS 里一个数字都不用记得乘倍率；位置按显示器分别记住；跟随活动显示器或固定在一块屏上。两项在设置窗口和托盘里都能改。四件真做了才浮出来的事。其一，**`display.id` 是个错的键** —— macOS 的 `CGDirectDisplayID` 会随拔插、换口、重启而变，而「按显示器记住位置」的全部价值恰恰在于**跨一次拔插仍然认得出来**；所以改用型号 + 分辨率 + 缩放倍率作键，两块一模一样的显示器（很常见的双屏配置）按摆放位置消歧 —— 于是在系统设置里把两块屏对调，位置记忆也跟着对调，而这恰好是对的：用户记住的是「左边那块屏的右下角」，不是某个序列号。其二，**位置必须存成工作区里的比例，不能存坐标** —— 绝对坐标只在「分辨率一辈子不变」时才成立，改一次缩放档位、接一次投影仪、系统更新换一次默认分辨率，那个点就落到屏幕外面去了。这正是*「换了显示器宠物就不见了」*的成因，也是为什么拔插之后必须主动重新落位：窗口的坐标还停在一片没有像素的桌面上，Electron 不会替你把它拉回来，而托盘看上去一切正常。其三，**跟随要的是「停留」而不是「越界」** —— 没有那 1.2 秒的停留门槛，任何一次横穿两块屏的鼠标移动都会把宠物拽走，而横穿是多屏用户每分钟都在做的事；宠物跟着光标乱窜比它不动更烦。其四，**钉住的那块屏被拔掉时不改钉** —— 宠物回主屏以保证看得见，但钉住的目标不改写，否则「固定在一块屏上」会在每次合盖之后悄悄换个含义；这段时间里设置窗口把宠物暂时去了哪儿说出来。这些全部是不 import Electron 的纯函数，因为它们每一条都**只在别人的桌上出错** | ~~3~~ **已完成** | ★ |
| 0.4 | ~~**自启 + 单命令启动**~~ ✅ | **9/5 已交付。** `npm start` 就是全部启动过程：托盘应用找到一个 ≥ 22.6 的 Node，拉起 Core，看着它（5 次重拉、指数退避，然后**停下**而不是一直循环），退出时收摊；再加设置 → 启动和托盘里的登录项。三件真做了才浮出来的事。其一，**已经在跑的 Core 必须「沿用」而不是「接管」** —— 隔壁终端里的 `npm run core:watch` 就是开发流程，退出时把它杀掉等于杀掉开发者正在盯的那个进程；所以沿用的 Core 永远不收，改为**探活**：不是你拉起来的进程不会给你 exit 事件，没有这一层轮询，用户在那个终端里 Ctrl-C 一下，托盘上会继续写着「运行中」，底下却是一具尸体。其二，**找 Node 不能只看 `PATH`** —— 这个功能里最要紧的那次启动恰恰来自登录项，而 launchd 递给应用的 PATH 是 `/usr/bin:/bin:/usr/sbin:/sbin`，Homebrew、nvm、fnm、Volta、`n` 全都在它之外；不显式枚举的话，「开机自启」的实际效果是每天早上看到一次「Core 未连接」。其三，登录项**在 dev 模式下根本无法验证** —— 注册进去的会是 `node_modules/electron`；所以那个开关会自己禁用并就地说明原因，app 还在已挂载的 dmg 里运行时同理：那种登录项会随着弹出映像一起失效 | ~~2~~ **已完成** | ★ |
...
| **0.11** | ~~**subagent 感知状态**~~ ✅ | **9/13 已交付。** `sessions.subagent_count` 由 `SubagentStart`/`SubagentStop` 维护，把 `working` 劈成三档 —— 0 自己干、1 `delegating`、2+ `juggling` —— 并且按**整张桌面**聚合而不是按 session：两个 session 各派 1 个，跑着的就是 2 个，宠物该 juggling（按 session 分档正是 #862 错的地方）。两档都没有新立绘：借 `working` 的立绘，差别放在动作上 —— `delegating` 刻意**比 working 更慢**，活派出去了，它在盯着 —— 外加一个 subagent 一颗绕圈的小方块（上限五颗），确切数字放在浮层里（`多线并行 ×4`）。有三件事是真做了才浮出来的。其一，**计数一定会漂，渲染必须自己扛住**：漏掉的 `SubagentStop`（hook 超时、`kill -9`）永远不会补来，所以排水口有四个 —— 减法夹 0、session 生命周期事件归零，以及这两档是 `working` 的**细分**而不是它的同级：一个 15 分钟没干活的 session，计数挂着几个都显示 idle。最后这条正是这个功能不需要自己的超时常量的原因。其二，**#214 有一个没人写下来的延迟版**：分身在跑时不采信 `Stop` 只是容易的那一半；如果把那个标记留着、等计数归零再采信，宠物就会在最后一个分身返回的**那一秒**说「干完了，等你」—— 同一个 bug，晚一个事件。所以凡是和在跑的分身共存过的 ready 标记一律丢弃，落回 `working`；真正的一轮结束会重新置位，而少说一次「待命」比错说一次「干完了」便宜得多。其三，轨道是**唯一一个会跑出精灵包围盒的特效**，因此半径必须按 canvas 收口而不是按精灵摊开：一只 160px 的宠物居中放在 208px 的画布上，左右各只剩 24px，「盒子外三格」这个想当然的半径会把方块送出窗口边缘 —— 那里不是被裁剪，是直接消失，看起来像特效在随机丢帧。这条是被一个「遍历每只宠物 × 两种动作的每个相位」的测试抓出来的。 | ~~2~~ **已完成** | ★★ |
...
1. **先出 Tier 0（还剩约 16 天 —— 0.1 / 0.5 于 8/22、0.10 / 0.12 于 8/25、0.13 于 9/4、0.4 于 9/5、0.11 于 9/13 交付）。** 不可选。一只静音、不能缩放、没有放行按钮的宠物，会在任何人评估成长回路之前就先在打磨度上输掉。**从 0.9（可操作权限气泡）开始** —— 一个你必须离开应用才能处理的通知是半成品，而这也是 clawd 有而 Vibepaws 没有的最明显的东西。
